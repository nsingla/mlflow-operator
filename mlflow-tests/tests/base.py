...
class TestBase:
    ...
    def admin_user_context(self):
        ...
        if not Config.LOCAL:
            ...
            self.admin_client = set_user_context((Config.ADMIN_USERNAME, Config.ADMIN_PASSWORD))
    ...
    def _execute_test_steps(self, test_data: TestData) -> None:
        ...
        if test_data.workspace_to_use:
            mlflow.set_workspace(test_data.workspace_to_use)
        ...
        for i, step in enumerate(test_steps, 1):
            ...
            if step.validate_func:
                self._execute_validation(validate_func=step.validate_func, step_number=i, action_name=step.action_func.__name__)
