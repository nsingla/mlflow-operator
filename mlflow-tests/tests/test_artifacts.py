...
class TestMLflowArtifacts(TestBase):
    ...
    test_scenarios: ClassVar[list[TestData]] = [
        ...
        TestData(
            test_name="User with UPDATE permission can log and load models",
        ...
        TestData(
            ...
            user_info=UserInfo(workspace=Config.WORKSPACES[0], verbs=[KubeVerb.GET], resource_types=[ResourceType.EXPERIMENTS, ResourceType.JOBS]),
            workspace_to_use=Config.WORKSPACES[0],
            test_steps = [
                TestStep(action_func=action_start_run),
