...
def validate_authentication_denied(test_context: TestContext) -> None:
    ...
    if test_context.last_error is None:
        ...
        assert False, f"Action should have failed for unauthorized user '{user_name}', but succeeded"
    ...
    if error_code == ErrorCode.PERMISSION_DENIED:
        ...
        logger.info(f"Successfully validated action failure - access forbidden")
    elif error_code == ErrorCode.WORKSPACE_ACCESS_DENIED:
        logger.info(f"Successfully validated action failure - workspace access denied")
    else:
